...
if (button_changing) {
	if (keyboard_lastkey != 0) {
		...
		scr_ButtonSet(option_index, keyboard_lastkey);
		for (var i = 0; i < BUTTON.NUMBER; i++) {
			button[i] = scr_ButtonToString(global.button[i]);
		}
	...
	if (keyboard_check_pressed(global.button_menu[BUTTON_MENU.ACCEPT])) {
		switch (menu_index) {
			...
			case menu_sub_save:
			global.save_index = option_index;
			
			var saveExists
			saveExists = file_exists(string(global.save_index));
			...
			switch (option_index) {
				case 0: // continue
				var file = string(global.save_index);
				if (file_exists(file)) {
					scr_ReadGame(file);
					scr_LoadGame();
				}
			...
			switch (option_index) {
				...
				case 1: // yes
					scr_NewGame(string(global.save_index));
				break;
			}
			break;
			...
			switch (option_index) {
				...
				setting[option_index] = global.setting[SETTING.FULLSCREEN] ? "true" : "false";
				break;
				
				case 1: // toggle smoothing
				scr_SettingSet(SETTING.SMOOTHING, !global.setting[SETTING.SMOOTHING]);
				setting[option_index] = global.setting[SETTING.SMOOTHING] ? "true" : "false";
				break;
				
				case 5: // restore default settings
				scr_SettingSetDefault();
				setting[0] = global.setting[SETTING.FULLSCREEN] ? "true" : "false";
				setting[1] = global.setting[SETTING.SMOOTHING] ? "true" : "false";
				setting[2] = string(global.setting[SETTING.FRAMERATE]);
			...
			switch (option_index) {
				...
				case 9: // restore default button configuration
				scr_ButtonSetDefault();
				for (var i = 0; i < BUTTON.NUMBER; i++) {
					button[i] = scr_ButtonToString(global.button[i]);
				}
				break;
	...
	if (keyboard_check_pressed(global.button_menu[BUTTON_MENU.DECLINE])) {
		switch (menu_index) {
			...
			menu_index = 1;
			option_index = global.save_index;
			break;
	...
	if (buttonLeft || buttonRight) {
		if (menu_index == menu_sub_settings) {
			switch (option_index) {
				case 2: // set scale
				scr_SettingSet(SETTING.SCALE, clamp(global.setting[SETTING.SCALE] + (buttonRight - buttonLeft) / 2, 1, setting_scale_max));
				...
				scr_SettingSet(SETTING.MUSIC, global.setting[SETTING.MUSIC] + setting_music_change * (buttonRight - buttonLeft));
				setting[SETTING.MUSIC] = string(global.setting[SETTING.MUSIC]);
				break;
				
				case 5: // set sound effect volume
				scr_SettingSet(SETTING.SOUND, global.setting[SETTING.SOUND] + setting_sound_change * (buttonRight - buttonLeft));
